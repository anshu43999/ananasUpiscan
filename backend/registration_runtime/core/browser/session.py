...
def extract_chatgpt_access_token(page: Any, *, attempts: int = 30, delay: float = 2.0, log_fn: Callable[[str], None] | None = None) -> BrowserAccessTokenResult:
    ...
    """
    last = BrowserAccessTokenResult(status="SESSION_TOKEN_MISSING", retryable=True)
    for _ in range(max(1, int(attempts or 1))):
        try:
            ...
            http_status = int((result or {}).get("status") or 0)
            data = (result or {}).get("data")
            if isinstance(data, dict):
                token = str(data.get("accessToken") or data.get("access_token") or "")
                if token:
                    return BrowserAccessTokenResult(access_token=token, status="SESSION_TOKEN_OK", http_status=http_status, retryable=False)
            text = str((result or {}).get("text") or "")
            if text:
                try:
                    parsed = json.loads(text)
                    token = str(parsed.get("accessToken") or parsed.get("access_token") or "")
                    if token:
                        return BrowserAccessTokenResult(access_token=token, status="SESSION_TOKEN_OK", http_status=http_status, retryable=False)
                except Exception:
                    pass
            retryable = http_status in {0, 401, 403, 429, 500, 502, 503, 504}
            status = "SESSION_HTTP_RETRYABLE" if retryable else "SESSION_HTTP_FAILED"
            reason = text[:300] if text else f"/api/auth/session returned status {http_status} without accessToken"
            last = BrowserAccessTokenResult(status=status, http_status=http_status, failure_reason=reason, retryable=retryable)
        except Exception as exc:
            last = BrowserAccessTokenResult(status="SESSION_FETCH_ERROR", http_status=0, failure_reason=str(exc), retryable=True)
        time.sleep(delay)
    reason = last.failure_reason or last.status or "access token missing"
    if log_fn:
