...
func importOnce(db *sql.DB, backend string, rec Record, email, plan, now string) (accountPK int64, err error) {
	...
	if hasUPIScanAccountLibrary(tx, backend) {
		return importUPIScanOnce(tx, backend, rec, email, plan, now)
	}
...
func hasUPIScanAccountLibrary(tx *sql.Tx, backend string) bool {
	var n int
	var err error
	if store.IsPostgres(backend) {
		err = tx.QueryRow(`SELECT COUNT(*) FROM information_schema.tables WHERE table_name='account_library'`).Scan(&n)
	} else {
		err = tx.QueryRow(`SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name='account_library'`).Scan(&n)
	}
	return err == nil && n > 0
}

func accountKey(rec Record, email string) string {
	for _, value := range []string{rec.AccountID, email} {
		if text := strings.TrimSpace(value); text != "" {
			return text
		}
	}
	sum := sha256.Sum256([]byte(strings.TrimSpace(rec.AccessToken)))
	return hex.EncodeToString(sum[:])[:20]
}

func importUPIScanOnce(tx *sql.Tx, backend string, rec Record, email, plan, now string) (accountPK int64, err error) {
	key := accountKey(rec, email)
	accountID := strings.TrimSpace(rec.AccountID)
	password := strings.TrimSpace(rec.Password)
	token := strings.TrimSpace(rec.AccessToken)
	source := "go_email_protocol"
	channels := "[]"
	emptyJSON := "{}"

	var existingID int64
	qSel := store.Rebind(backend, `SELECT id FROM account_library WHERE account_key=? OR email=? LIMIT 1`)
	qerr := tx.QueryRow(qSel, key, email).Scan(&existingID)
	if qerr != nil && qerr != sql.ErrNoRows {
		return 0, qerr
	}
	if existingID > 0 {
		qUpd := store.Rebind(backend, `
			UPDATE account_library SET
				account_id=CASE WHEN ?!='' THEN ? ELSE account_id END,
			...
			WHERE id=?
		`)
		if _, err := tx.Exec(qUpd, accountID, accountID, email, email, password, password, token, plan, source, now, existingID); err != nil {
			return 0, err
		}
		accountPK = existingID
	} else {
		qIns := store.Rebind(backend, `
			INSERT INTO account_library(
				...
				plan_type, status, source, channels_json,
				...
				note, plus_status, plus_verified_at, plus_check_source, plus_check_error, plus_json,
				created_at, updated_at
			) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
		`)
		res, e := tx.Exec(qIns,
			key, accountID, email, password, token, "",
			plan, "active", source, channels,
			"unknown", "", emptyJSON, "",
			"unknown", "", "", "", emptyJSON,
			"", "unknown", "", "", "", emptyJSON,
			now, now,
		...
		if store.IsPostgres(backend) {
			if e := tx.QueryRow(store.Rebind(backend, `SELECT id FROM account_library WHERE account_key=? LIMIT 1`), key).Scan(&accountPK); e != nil {
				return 0, e
			}
		} else {
			accountPK, _ = res.LastInsertId()
