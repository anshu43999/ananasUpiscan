...
func CreateRegistrationTasks(dbPath, batchID string, count int) ([]string, error) {
	...
	if !hasTaskTables(db, backend) {
		ids := make([]string, 0, count)
		for range count {
			ids = append(ids, "goep_"+uuid.NewString())
		}
		return ids, nil
	}
...
func hasTaskTables(db *sql.DB, backend string) bool {
	var taskCount int
	var eventCount int
	if store.IsPostgres(backend) {
		err1 := db.QueryRow(`SELECT COUNT(*) FROM information_schema.tables WHERE table_name='tasks'`).Scan(&taskCount)
		err2 := db.QueryRow(`SELECT COUNT(*) FROM information_schema.tables WHERE table_name='task_events'`).Scan(&eventCount)
		return err1 == nil && err2 == nil && taskCount > 0 && eventCount > 0
	}
	err1 := db.QueryRow(`SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name='tasks'`).Scan(&taskCount)
	err2 := db.QueryRow(`SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name='task_events'`).Scan(&eventCount)
	return err1 == nil && err2 == nil && taskCount > 0 && eventCount > 0
}
